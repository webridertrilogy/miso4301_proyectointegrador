...
public class PeticionConflictoHorarioFacadeTest {
	...
	@EJB
	private PeticionConflictoHorarioFacadeLocal facade;
	
	@Test
	public void testAll() throws SQLException {
		PeticionConflictoHorario av = new PeticionConflictoHorario();
		
		facade.create(av);
		av.setComentarios("aa");
		facade.edit(av);
		facade.remove(av);
		facade.find(1L);
		facade.buscarPorEstudianteYSeccionDestino("aaa", 1L);
		facade.buscarPorCorreo("a");
		facade.buscarPorIdYCorreo("1", 1L);
		facade.buscarPorCodigoCurso("1");
		facade.buscarPorCodigoCursoYTipo("1", "1", 1, 5);
		facade.buscarPeticionesResueltas();
		facade.buscarPorSeccion(1L);
