...
public class EventoExternoFacadeTest {
	...
	@EJB
	private EventoExternoFacadeLocal facade;
	
	@Test
	public void testAll() throws SQLException {
		EventoExterno av = new EventoExterno();
		facade.create(av);
		av.setCupo(1);
		facade.edit(av);
		facade.remove(av);
		facade.find(1L);
		facade.findInscritosByIdEvento(1L);
		facade.findEventosByIdInscripcion(1L);
		facade.findEventosByIdCategoria(1L);
		facade.findInscritoByIdEventoAndIdContacto(1L, 1L);
		facade.findEventosByTitulo("q");
		facade.findContactosEventoExterno(1L, "a", "a", "a", "a", "aa", "a", "a", "a", "a", new Date());
		facade.findEventosByEstado("a");
	}
}