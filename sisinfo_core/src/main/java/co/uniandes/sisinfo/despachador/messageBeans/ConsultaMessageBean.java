 ...
 * @author JuanCamilo
 */
@MessageDriven(mappedName = "ColaConsulta", activationConfig =  {
...
public class ConsultaMessageBean implements MessageListener {
    ...
    public ConsultaMessageBean() {
        try {
            parserBean = new ParserT();
            serviceLocator = new ServiceLocator();
            constanteBean = (ConstanteRemote) serviceLocator.getRemoteEJB(ConstanteRemote.class);
        } catch (NamingException ex) {
            Logger.getLogger(ConsultaMessageBean.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public void onMessage(Message message) {
        ...
        try {
            ...
            if(!redelivered)
            {
                ...
                if(nombreComando.equals(getConstanteBean().getConstante(Constantes.CMD_MODIFICAR_HORARIO)))
                {
                    respuesta = consultaBean.modificarHorario(comandoXML);
                }
