 ...
 * @author ale-osor
 */
@MessageDriven(mappedName = "ColaAspirante", activationConfig =  {
...
public class AspiranteMessageBean implements MessageListener {
    ...
    public AspiranteMessageBean() {
        try {
            parserBean = new ParserT();
            serviceLocator = new ServiceLocator();
            constanteBean = (ConstanteRemote) serviceLocator.getRemoteEJB(ConstanteRemote.class);
        } catch (NamingException ex) {
            Logger.getLogger(AspiranteMessageBean.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public void onMessage(Message message) {
        ...
        try {
            ...
            if(!redelivered)
            {
                ...
                if(nombreComando.equals(getConstanteBean().getConstante(Constantes.CMD_ACTUALIZAR_DATOS_ACADEMICOS)))
                {
                    respuesta = aspiranteBean.actualizarDatosAcademicos(comandoXML);
                }
                else if(nombreComando.equals(getConstanteBean().getConstante(Constantes.CMD_ACTUALIZAR_DATOS_PERSONALES_Y_EMERGENCIA)))
                {
                    respuesta = aspiranteBean.actualizarDatosPersonalesYEmergencia(comandoXML);
                }                
            ...
            Logger.getLogger(ListaNegraMessageBean.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
