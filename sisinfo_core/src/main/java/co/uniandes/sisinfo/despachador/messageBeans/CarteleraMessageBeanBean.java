 ...
 * @author ale-osor
 */
@MessageDriven(mappedName = "ColaCartelera", activationConfig =  {
...
public class CarteleraMessageBeanBean implements MessageListener {
    ...
    public CarteleraMessageBeanBean() {
        try {
            parserBean = new ParserT();
            serviceLocator = new ServiceLocator();
            constanteBean = (ConstanteRemote) serviceLocator.getRemoteEJB(ConstanteRemote.class);
        } catch (NamingException ex) {
            Logger.getLogger(CarteleraMessageBeanBean.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public void onMessage(Message message) {
        ...
        try {
            ...
            idMessage = message.getJMSCorrelationID();
            ...
            if(!redelivered)
            {
                ...
                if(nombreComando.equals(getConstanteBean().getConstante(Constantes.CMD_CARGAR_CARTELERA)))
                {
                    respuesta = carteleraBean.cargarCartelera(comandoXML);
                }
                if(respuesta == null)
                    respuesta =" ";
