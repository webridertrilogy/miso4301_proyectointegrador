...
public class ColaProfesorBean implements ColaProfesorLocal {
    ...
    public String resolverComando(String comandoXML) {
        ...
        try {
            ...
            Logger.getLogger(ColaListaNegraBean.class.getName()).log(Level.SEVERE, null, ex);
        }
        return respuesta;
    }
    ...
    private String enviarMensajeACola(String comandoXML)throws JMSException{
        ...
        try {
            ...
            messageRequested.setJMSReplyTo(replyColaProfesor);

            MessageProducer messageProducer = session.createProducer(colaProfesor);
            ...
            connection.start();
            session = connection.createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
            MessageConsumer messageConsumer = session.createConsumer(replyColaProfesor);
            ...
            while (!idMessageReplied.equals(idMessageRequested)) {
                ...
                connection.start();
                session = connection.createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
                messageProducerReply = session.createProducer(replyColaProfesor);
                ...
                connection.start();
                session = connection.createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
                messageConsumer = session.createConsumer(replyColaProfesor);
            ...
            Logger.getLogger(ColaProfesorBean.class.getName()).log(Level.SEVERE, null, ex);
