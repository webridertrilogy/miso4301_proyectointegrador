...
public class ColaConvocatoriaBean implements ColaConvocatoriaLocal {
    ...
    public String resolverComando(String comandoXML) {
        ...
        try {            
            ...
            Logger.getLogger(ColaConvocatoriaBean.class.getName()).log(Level.SEVERE, null, ex);
        }
        return respuesta;
    }
    ...
    private String enviarMensajeACola(String comandoXML)throws JMSException{
        ...
        try {
            ...
            messageRequested.setJMSReplyTo(replyColaConvocatoria);

            MessageProducer messageProducer = session.createProducer(colaConvocatoria);            
            ...
            connection.start();
            session = connection.createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
            MessageConsumer messageConsumer = session.createConsumer(replyColaConvocatoria);
            ...
            while (!idMessageReplied.equals(idMessageRequested)) {
                ...
                connection.start();
                session = connection.createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
                messageProducerReply = session.createProducer(replyColaConvocatoria);
                ...
                connection.start();
                session = connection.createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
                messageConsumer = session.createConsumer(replyColaConvocatoria);
