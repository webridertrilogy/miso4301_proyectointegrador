...
public class ColaListaNegraBean implements ColaListaNegraLocal {
    ...
    private String enviarMensajeACola(String comandoXML)throws JMSException{
        ...
        try {
            ...
            messageRequested.setJMSReplyTo(replyColaListaNegra);

            MessageProducer messageProducer = session.createProducer(colaListaNegra);
            ...
            connection.start();
            session = connection.createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
            MessageConsumer messageConsumer = session.createConsumer(replyColaListaNegra);
            ...
            while (!idMessageReplied.equals(idMessageRequested)) {
                ...
                connection.start();
                session = connection.createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
                messageProducerReply = session.createProducer(replyColaListaNegra);
                ...
                connection.start();
                session = connection.createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
                messageConsumer = session.createConsumer(replyColaListaNegra);
