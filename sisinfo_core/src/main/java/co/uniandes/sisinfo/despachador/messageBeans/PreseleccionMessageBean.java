 ...
 * @author ale-osor
 */
@MessageDriven(mappedName = "ColaPreseleccion", activationConfig =  {
...
public class PreseleccionMessageBean implements MessageListener {
    ...
    public PreseleccionMessageBean() {
        try {
            parserBean = new ParserT();
            serviceLocator = new ServiceLocator();
            constanteBean = (ConstanteRemote) serviceLocator.getRemoteEJB( ConstanteRemote.class);
        } catch (NamingException ex) {
            Logger.getLogger(PreseleccionMessageBean.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public void onMessage(Message message) {
        ...
        try {
            ...
            if(!redelivered)
            {
                ...
                if(nombreComando.equals(getConstanteBean().getConstante(Constantes.CMD_ENVIAR_PRESELECCION)))
                {
                    respuesta = preseleccionBean.preseleccionar(comandoXML);
