 ...
 * @author JuanCamilo
 */
@MessageDriven(mappedName = "ColaProfesor", activationConfig =  {
...
public class ProfesorMessageBean implements MessageListener {
    ...
    public ProfesorMessageBean() {
        try {
            parserBean = new ParserT();
            serviceLocator = new ServiceLocator();
            constanteBean = (ConstanteRemote) serviceLocator.getRemoteEJB(ConstanteRemote.class);
            profesorBean = (ProfesorRemote)serviceLocator.getRemoteEJB(ProfesorRemote.class);
        } catch (NamingException ex) {
            Logger.getLogger(ProfesorMessageBean.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public void onMessage(Message message) {
        ...
        try {
            ...
            if(!redelivered)
            {
                ...
                if(nombreComando.equals(getConstanteBean().getConstante(Constantes.CMD_AGREGAR_PROFESOR)))
                {
                    respuesta = profesorBean.agregarProfesor(comandoXML);
                }
