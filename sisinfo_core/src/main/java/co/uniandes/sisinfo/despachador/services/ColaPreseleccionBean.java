...
public class ColaPreseleccionBean implements ColaPreseleccionLocal {
    ...
    public String resolverComando(String comandoXML) {
        ...
        try {
            ...
            Logger.getLogger(ColaPreseleccionBean.class.getName()).log(Level.SEVERE, null, ex);
        }
        return respuesta;
    }
    ...
    private String enviarMensajeACola(String comandoXML)throws JMSException{
        ...
        try {
            ...
            messageRequested.setJMSReplyTo(replyColaPreseleccion);

            MessageProducer messageProducer = session.createProducer(colaPreseleccion);
            ...
            connection.start();
            session = connection.createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
            MessageConsumer messageConsumer = session.createConsumer(replyColaPreseleccion);
            ...
            while (!idMessageReplied.equals(idMessageRequested)) {
                ...
                connection.start();
                session = connection.createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
                messageProducerReply = session.createProducer(replyColaPreseleccion);
                ...
                connection.start();
                session = connection.createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
                messageConsumer = session.createConsumer(replyColaPreseleccion);
