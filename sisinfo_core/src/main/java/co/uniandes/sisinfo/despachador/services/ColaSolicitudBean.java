...
public class ColaSolicitudBean implements ColaSolicitudLocal {
    ...
    public String resolverComando(String comandoXML) {
        ...
        try {
            ...
            Logger.getLogger(ColaSolicitudBean.class.getName()).log(Level.SEVERE, null, ex);
        }
        return respuesta;
    }
    private String enviarMensajeACola(String comandoXML)throws JMSException{
        ...
        try {
            ...
            messageRequested.setJMSReplyTo(replyColaSolicitud);

            MessageProducer messageProducer = session.createProducer(colaSolicitud);
            ...
            connection.start();
            session = connection.createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
            MessageConsumer messageConsumer = session.createConsumer(replyColaSolicitud);
            ...
            while (!idMessageReplied.equals(idMessageRequested)) {
                ...
                connection.start();
                session = connection.createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
                messageProducerReply = session.createProducer(replyColaSolicitud);
                ...
                connection.start();
                session = connection.createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
                messageConsumer = session.createConsumer(replyColaSolicitud);
