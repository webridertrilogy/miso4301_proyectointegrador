...
public class ColaConvenioBean implements ColaConvenioLocal {
    ...
    public String resolverComando(String comandoXML) {
        ...
        try {
            ...
            Logger.getLogger(ColaConvenioBean.class.getName()).log(Level.SEVERE, null, ex);
        }
        return respuesta;
    }
    ...
    private String enviarMensajeACola(String comandoXML)throws JMSException{
        ...
        try {
            ...
            messageRequested.setJMSReplyTo(replyColaConvenio);

            MessageProducer messageProducer = session.createProducer(colaConvenio);
            ...
            connection.start();
            session = connection.createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
            MessageConsumer messageConsumer = session.createConsumer(replyColaConvenio);
            ...
            while (!idMessageReplied.equals(idMessageRequested)) {
                ...
                connection.start();
                session = connection.createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
                messageProducerReply = session.createProducer(replyColaConvenio);
                ...
                connection.start();
                session = connection.createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
                messageConsumer = session.createConsumer(replyColaConvenio);
