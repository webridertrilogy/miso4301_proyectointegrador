...
public class ColaReglaBean implements ColaReglaLocal {
    ...
    private String enviarMensajeACola(String comandoXML)throws JMSException{
        ...
        try {
            ...
            messageRequested.setJMSReplyTo(replyColaRegla);

            MessageProducer messageProducer = session.createProducer(colaRegla);
            ...
            connection.start();
            session = connection.createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
            MessageConsumer messageConsumer = session.createConsumer(replyColaRegla);
            ...
            while (!idMessageReplied.equals(idMessageRequested)) {
                ...
                connection.start();
                session = connection.createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
                messageProducerReply = session.createProducer(replyColaRegla);
                ...
                connection.start();
                session = connection.createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
                messageConsumer = session.createConsumer(replyColaRegla);
