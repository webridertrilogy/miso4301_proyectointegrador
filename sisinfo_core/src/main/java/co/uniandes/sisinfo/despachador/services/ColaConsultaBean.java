...
public class ColaConsultaBean implements ColaConsultaLocal {
    ...
    public String resolverComando(String comandoXML) {
        ...
        try {
            ...
            Logger.getLogger(ColaConsultaBean.class.getName()).log(Level.SEVERE, null, ex);
        }
        return respuesta;
    }
    ...
    private String enviarMensajeACola(String comandoXML)throws JMSException{
        ...
        try {
            ...
            messageRequested.setJMSReplyTo(replyColaConsulta);

            MessageProducer messageProducer = session.createProducer(colaConsulta);
            ...
            connection.start();
            session = connection.createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
            MessageConsumer messageConsumer = session.createConsumer(replyColaConsulta);
            ...
            while (!idMessageReplied.equals(idMessageRequested)) {
                ...
                connection.start();
                session = connection.createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
                messageProducerReply = session.createProducer(replyColaConsulta);
                ...
                connection.start();
                session = connection.createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
                messageConsumer = session.createConsumer(replyColaConsulta);
